...
public class Dbmain_Temp5_HW {
	public static void main(String[] args){
			...
			try{
				...
				book_genre = "한국소설";
				book_price = "14,000";
				Mupdate_HW mupdate = new Mupdate_HW();
				flag = mupdate.mupdate(book_name, book_author, book_publish, book_genre, book_price);
				System.out.println("수정결과 보기");
				booklists = mselect.mselect("", book_name);
				for(int i =0; i<booklists.size(); i++) {
					System.out.println(booklists.get(i).getBook_name()+","+booklists.get(i).getBook_publish());
				}
				System.out.println("삭제합니다");
				book_name = "울타리의 노래";
				mdelete = new Mdelete_HW();
				flag = mdelete.mdelete("",book_name);
				System.out.println("삭제결과 보기");
				booklists = mselect.mselect("all", book_name);
				for(int i =0; i<booklists.size(); i++) {
					System.out.println(booklists.get(i).getBook_name()+","+booklists.get(i).getBook_publish());
				}
			}
